use crate::command::util::{
    ...
    require_opensquare_votes, require_subsquare_referendum, require_thread,
};
    ...
    pub(crate) async fn process_feedback_summary_command(
        ...
        if opensquare_votes.is_empty() {
            self.telegram_client
                .send_message(chat_id, Some(thread_id), "No votes yet.", true)
                .await?;
            return Ok(());
        }
        ...
        let summary = self
            ...
            .await?;
        self.telegram_client
            .send_message(chat_id, Some(thread_id), &summary, true)
            .await?;
        Ok(())
    }
}
